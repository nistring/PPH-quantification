...
class PPHBatchProcessor:
    ...
    def _process_parallel(self, patients, output_dir, max_workers):
        ...
        if max_workers is None:
            max_workers = min(len(patients), multiprocessing.cpu_count() // 2)  # Use fewer cores to avoid memory issues
...
def create_summary_report(results, output_dir):
    ...
    # Prepare data with per-phase columns
    summary_data = []
    for r in results:
        row = {
            "patient": r["patient_name"],
            "processing_time": f"{r['processing_time']:.1f}s",
            "arterial_volume_ml": r["hemorrhage_volume_arterial"],
            "portal_volume_ml": r["hemorrhage_volume_portal"],
            "subtract_volume_ml": r["hemorrhage_volume_subtract"]
        }
...
def main():
    ...
    parser.add_argument("--input-dir", "-i", default="extracted_series", help="Input directory")
    ...
    parser.add_argument("--workers", "-w", default=8, type=int, help="Number of workers")
